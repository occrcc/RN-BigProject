...
-(void)example06_imageAd_defaultConfiguration
{
  ...
  CGFloat height = WIGHT/1242*1786 ;
    CGFloat footViewConstant = [[UIApplication sharedApplication] statusBarFrame].size.height>20?20:0;
    if (footViewConstant == 20) {
        height += 100;
    }
    
    imageAdconfiguration.frame = CGRectMake(0, 0, WIGHT, height);
    //广告图片URLString/或本地图片名(.jpg/.gif请带上后缀)
  
  NSString *imgUrl =  [[NSUserDefaults standardUserDefaults] objectForKey:@"startImage"];
  imageAdconfiguration.imageNameOrURLString = imgUrl;
    
    //广告点击打开链接
//    imageAdconfiguration.openURLString = NULL;
    [XHLaunchAd imageAdWithImageAdConfiguration:imageAdconfiguration delegate:self];
}
/**
 *  视频
